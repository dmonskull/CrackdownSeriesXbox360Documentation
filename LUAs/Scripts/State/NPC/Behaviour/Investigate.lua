...
--	Description: Walk to the position of a suspicious sound, look around,
-- then give up
-- Owner: Nathan
...
require "State\\NPC\\Action\\Idle\\Speak"
require "State\\NPC\\Action\\Movement\\Move"
require "State\\NPC\\Behaviour\\Scripted\\LookAround"

Investigate = Create (State, 
{
	sStateName = "Investigate",
	bCanInterrupt = true,
	nInterruptTimeout = 2,
})

function Investigate:OnEnter ()
	-- Call parent
	State.OnEnter (self)
	...
	{
		vTargetPosition = self.vPosition,
	}))
end

function Investigate:OnEvent (tEvent)
	...
	elseif tEvent:HasID (self.nSoundID) then
		...
		if self.bCanInterrupt and self:IsSoundSuspicious (self.tHost, tEvent:RetSource ()) then
			self.tHost:SpeakAudio (eVocals.nInvestigating, "I thought I heard a puddy-tat!")
			self:OnNPCHeardSuspiciousSound (self.tHost, tEvent:RetPosition ())
		end
		return true
...
function Investigate:OnActiveStateFinished ()
	...
	elseif tState:IsA (LookAround) then
		...
		self:ChangeState (Create (Speak, 
		{
			sSentence = "There's nothing there now",
		}))
		return true

	elseif tState:IsA (Speak) then
...
function Investigate:IsValidPosition (vPosition)
...
function Investigate:OnNPCHeardSuspiciousSound (tNPC, vPosition)
...
function Investigate:ResetInterruptTimer ()
