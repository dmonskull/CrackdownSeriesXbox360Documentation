...
require "State\\Crime\\GangHarassment\\PreCrime"
require "State\\Crime\\GangHarassment\\Crime"
require "State\\Crime\\GangHarassment\\Reset"
require "State\\Crime\\GangHarassment\\HarassEnemy"
require "State\\Crime\\GangHarassment\\AttackEnemy"
require "State\\NPC\\Character\\Pedestrian"
require "State\\NPC\\Character\\StreetSoldier"

namespace ("GangHarassment")

GangHarassment = Create (AmbientCrimeState,
{
	sStateName = "GangHarassment",
})

function GangHarassment:OnEnter ()
	-- Call parent
	AmbientCrimeState.OnEnter (self)
	Emit ("test string 2 " .. self.tHost:RetName ())
end

function GangHarassment:Construct (tParameters)
	...
	for i=1, self.tParameters.nMaxGangMembers do
		...
		-- Spawn the character, give him a gun
	...
	-- Subscribe events
	self.nZoneAIPlayerID = self:Subscribe (eEventType.AIE_ZONE_AIPLAYER, tTriggerZone)
...
function GangHarassment:OnExit ()
...
function GangHarassment:OnActiveStateFinished ()
...
function GangHarassment:OnEvent (tEvent)
...
function GangHarassment:IsCrimeWithinActivityVolume ()
...
-- Pedestrian entered the trigger zone
function GangHarassment:OnDetectedPedestrian (tPedestrian)
...
-- Streetsoldier entered the trigger zone
function GangHarassment:OnDetectedStreetSoldier (tStreetSoldier)
...
function GangHarassment:OnNPCHeardSuspiciousSound (tNPC, vPosition)
end

-- Took damage from an enemy - the attacker's location is not necessarily known
function GangHarassment:OnNPCAttacked (tNPC, tAttacker)
...
function GangHarassment:OnNPCDetectedEnemy (tNPC, tEnemy)
...
-- NPC died
function GangHarassment:OnNPCDied (tNPC, tKiller)
	self:OnLostGangMember (tNPC)
end

function GangHarassment:OnAddGangMember (tGangMember)
...
function GangHarassment:OnLostGangMember (tGangMember)
	-- Unsubscribe events
...
function GangHarassment:RetNumGangMembers ()
...
-- number appended to it. For example: 001_GangHarassment_NPC_SpawnPoint_01 
-- would be the first available spawn point
function GangHarassment:GenerateSpawnPoints (nNumGangMembers)
